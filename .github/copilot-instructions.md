...
<activity android:name=".core.cielo.ResponseActivity" android:exported="true"
...
Whenever making code changes or adding new features, AI agents **MUST** run and pass the following
quality gates in PowerShell/Terminal:
...
- **Zero Unused Imports & Clean Syntax:** Always format code adhering to `ktlint` rules before
  completing a task.
- **Strict Architecture Boundaries:** Never import data layer classes directly into presentation
  layer Composables. Always pass through Domain Use Cases/Repositories.
- **Idempotency Enforcement:** Ensure any code modifying transaction states uses Room transactions (
  `@Transaction`) to guarantee thread safety and data integrity.
